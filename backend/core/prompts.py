...
4. 信息冲突时，以文档段落原文为准
5. 若信息不足，明确说明
6. 使用 Markdown 格式输出，包含标题、列表、表格等
...
# ── KG 实体抽取 ──
KG_EXTRACT_PROMPT = """\
你是一个知识图谱实体抽取专家。从以下文本中抽取重要的命名实体。

文本：
{text}

请抽取实体并以 JSON 数组格式输出，每个实体包含：
- entity_text: 实体名称（必须是文本中的原文）
- entity_class: 实体类型（product/concept/technology/organization/person/framework/model/other）
- attributes: 该实体的关键属性键值对（从文本中提取，至少1个）
...
注意：
1. 只抽取文本中明确提及的实体，不要推断
2. 实体名必须是原文的精确子串
3. 每个 entity_text 必须能在原文中找到（用于字符定位）
4. 最多抽取 15 个最重要的实体"""
